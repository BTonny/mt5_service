...
if ! WINEARCH=win64 WINEPREFIX=/config/.wine $wine_executable python --version > /dev/null 2>&1; then
    log_message "INFO" "Installing Python in Wine..."
    wget -O /tmp/python-installer.exe $python_url > /dev/null 2>&1
    WINEARCH=win64 WINEPREFIX=/config/.wine $wine_executable /tmp/python-installer.exe /quiet InstallAllUsers=1 PrependPath=1 Include_test=0
